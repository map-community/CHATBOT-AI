...
echo ""
echo "⚠️  중요: Flask 앱 재시작 필요!"
echo "   MongoDB 데이터가 변경되었으므로 Redis 캐시를 새로 생성해야 합니다."
echo ""
echo "📋 다음 단계 (EC2에서 실행):"
echo "   1. SSH 접속: ssh -i $EC2_KEY $EC2_USER@$EC2_HOST"
echo "   2. 앱 재시작: docker restart knu-chatbot-app"
echo "   3. 로그 확인: docker logs -f knu-chatbot-app"
echo ""
echo "💡 또는 한 번에 실행:"
echo "   ssh -i $EC2_KEY $EC2_USER@$EC2_HOST 'docker restart knu-chatbot-app'"
echo ""
