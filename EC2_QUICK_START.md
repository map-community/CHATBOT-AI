...
| 옵션 | 인스턴스 | 월 비용 | 메모리 | 적합 |
|------|----------|---------|--------|------|
| **💰 초저비용** | **t3.small** | **$15** | **2GB** | **트래픽 적음** ⭐ |
| 💡 최고 가성비 | t4g.small (ARM) | $12 | 2GB | 트래픽 적음 |
| 🚀 최저가 | t3.medium Spot | $5-10 | 4GB | 개발/테스트 |
| 안정적 (전체 기능) | t3.large | $60 | 8GB | Reranker 포함 |

2. **보안 그룹**: 22(SSH), 80(HTTP), 443(HTTPS) 포트 열기
3. **도메인** (선택사항): HTTPS 사용 시 필요

> 💡 **비용 절감:** Reranker 제거 시 t3.small (2GB, $15/월)로 충분!
>
> Reranker는 검색 정확도를 높이지만 필수는 아닙니다. BM25 + Dense 검색만으로도 충분히 작동합니다.
>
> - **Reranker 포함**: 4-7.5GB 메모리 필요 → t3.large (8GB, $60/월)
> - **Reranker 제거**: 1.5-2GB 메모리 → t3.small (2GB, $15/월) ✅
>
...
**t3.small (2GB) 사용 시 권장:**

```bash
# requirements-minimal.txt 사용 (Reranker 제거)
cp requirements-minimal.txt requirements.txt

# 또는 수동으로 FlagEmbedding 제거
sed -i 's/^FlagEmbedding/#FlagEmbedding/' requirements.txt
```

이제 메모리 사용량이 1.5-2GB로 줄어듭니다! ✅
