...
크롤러 실행 시 자동으로 로그 파일이 생성됩니다:
- **메인 로그**: `logs/crawl_YYYY-MM-DD_HH-MM-SS.txt`
- **부분 실패 로그**: `logs/partial_failures_YYYY-MM-DD_HH-MM-SS.json`
...
```
✅ 성공: 2024학년도 학사 일정 안내
   URL: https://cse.knu.ac.kr/bbs/board.php?bo_table=sub5_1&wr_id=1234
   처리 내용: 텍스트 1234자, 이미지 3개, 첨부파일 1개, 임베딩 5개
   ...
   📋 저장 데이터 샘플 (첫 번째 아이템):
      ...
      텍스트 길이: 850자
      HTML 구조: ❌ 없음 (평문 텍스트만)
      텍스트 미리보기: 2024학년도 1학기 학사 일정을 안내합니다...
```
...
📝 텍스트 필드:
   길이: 234자
   미리보기: [이미지 텍스트] 개강일: 3월 2일 종강일: 6월 15일...

🌐 HTML 구조 필드:
   ✅ 저장됨 (567자)
   용도: 표, 레이아웃 맥락 보존 → RAG 품질 향상
   미리보기: <table><tr><td>개강일</td><td>3월 2일</td></tr>...
   HTML 활용 가능: ✅

🖼️ 이미지 URL: https://cse.knu.ac.kr/data/editor/2511/image.png
================================================================================
```
...
✅ 성공: 입학 설명회 안내
   ...
   처리 내용: 텍스트 500자, 이미지 5개, 첨부파일 2개, 임베딩 8개
   ⚠️  이미지 OCR 실패: 2개
   ⚠️  첨부파일 파싱 실패: 1개
```
...
✅ 로그 파일 저장 완료: logs/crawl_2024-03-01_12-34-56.txt
```
  ...
  "게시글_목록": [
    {
      ...
      "이미지": {
        ...
        "실패_목록": [
          {
            "URL": "https://cse.knu.ac.kr/data/editor/image1.png",
            "사유": "이미지 다운로드 실패"
          },
          {
            "URL": "https://cse.knu.ac.kr/data/editor/image2.png",
            "사유": "corrupted document"
          }
        ]
      },
      "첨부파일": {
        ...
        "실패_목록": [
          {
            "URL": "https://cse.knu.ac.kr/data/file/document.pdf",
            "사유": "PDF 파싱 실패"
          }
        ]
      },
      "요약": "이미지 OCR 실패 2개 / 첨부파일 파싱 실패 1개"
    }
  ]
}
```
...
```bash
# 부분 실패 로그 확인
cat logs/partial_failures_2024-03-01_12-34-56.json | jq

# 특정 게시글의 실패 원인 확인
cat logs/partial_failures_2024-03-01_12-34-56.json | jq '.게시글_목록[0]'

# 실패한 이미지 URL만 추출
cat logs/partial_failures_2024-03-01_12-34-56.json | jq '.게시글_목록[].이미지.실패_목록[].URL'
```

### 로그 활용 팁

1. **데이터 품질 확인**: 각 게시글의 HTML 구조 저장 여부 확인
2. **실패 원인 분석**: 부분 실패 로그에서 실패 패턴 파악
3. **재처리 대상 선별**: 일시적 오류는 재크롤링, 구조적 문제는 무시
4. **비용 최적화**: HTML 구조가 저장되는지 확인하여 API 비용 정당화
...
2. **알림**: 크롤링 완료 시 알림 (Slack, Email 등)
3. **모니터링**: 크롤링 실패 감지 및 재시도
4. **분산 처리**: 여러 서버에서 병렬 크롤링
