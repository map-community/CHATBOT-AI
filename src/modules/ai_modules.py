...
3. **답변 완전성 vs 간결성 (중요!):**
   - **명단/목록 질문**: "누구", "대상자", "명단", "리스트", "목록" 등이 포함된 질문은 문서의 **모든 항목을 빠짐없이 나열**하세요.
     * 예: "면접 대상자 누구니?" → 문서에 있는 모든 대상자를 전부 나열
     * 예: "장학금 수혜자는?" → 모든 수혜자 이름과 학번을 전부 제공
   - **일반 질문**: 명단이 아닌 일반 질문은 간결하게 답변하세요.
